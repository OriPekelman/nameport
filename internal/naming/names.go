...
// Blacklisted paths - services from these locations will be ignored
var blacklistedPaths = []string{
	// Core system services that shouldn't be proxied
	"/usr/libexec/",
	"/usr/lib/",
	"/private/var/",
	"/var/",
...
// Blacklisted executable names - services with these names will be ignored
var blacklistedNames = []string{
	"localhost-magic-daemon",
	"localhost-magic",
}

// IsOwnExecutable checks if the executable is this daemon itself
func IsOwnExecutable(exePath string) bool {
	// Check if path contains our name
	if strings.Contains(exePath, "localhost-magic-daemon") {
		return true
	}
	if strings.Contains(exePath, "localhost-magic") {
		return true
	}
	return false
}

// Blacklisted name patterns - names matching these patterns will be ignored
var blacklistedPatterns = []string{
	`^localhost-magic`,
}

// IsBlacklisted checks if a service should be ignored
func IsBlacklisted(exePath string, args []string) bool {
	// Check if this is our own executable
	if IsOwnExecutable(exePath) {
		return true
	}

	// Check blacklisted executable names
	...
	if isInterpreter(exeName) && len(args) > 1 {
		scriptPath := args[1]
		// If script is in a user directory, don't blacklist
	...
	for _, prefix := range blacklistedPaths {
		if strings.HasPrefix(exePath, prefix) {
			// Exception: allow user apps in /Applications even though /Applications starts with /App
...
// isInterpreter checks if the executable is a language interpreter
func isInterpreter(name string) bool {
	interpreters := []string{"python", "python3", "node", "nodejs", "ruby", "perl", "php", "java"}
...
// IsNameBlacklisted checks if a generated name should be ignored
